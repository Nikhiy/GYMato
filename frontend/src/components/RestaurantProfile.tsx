...
const RestaurantProfile=({restaurant,isSeller,onUpdate}:props)=>{
  ...
  const saveChanges=async()=>{
    try{
      ...
      setEditMode(false); // ✅ CHANGED: exit edit mode after saving
    }catch(error){
      console.log(error)
  ...
  return (
    <div className="mx-auto max-w-xl rounded-xl bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-5 space-y-4">
        ...
        {
          editMode ?( <textarea value={description} onChange={(e)=>setDescription(e.target.value)} className="w-full rounded border px-3 py-2 text-sm" />):(<p className="text-sm text-gray-600">{restaurant.description || "No descriptionadded"}</p>
        )}
        
        <div className="flex items-center justify-between pt-3 border-t">
          <span className={`text-sm font-medium${isOpen?"text-green-600":"text-red-500"}`}>{isOpen?"OPEN":"CLOSED"}
          </span>
          ...
          <div className="flex gap-3">
            ...
            {isSeller && (
              <button
                ...
                }`} // ✅ CHANGED: fixed broken template string
              >
        ...
        <p className="text-sm text-gray-400">Created on {new Date(restaurant.createdAt).toLocaleDateString()}</p>
      </div>
    </div>
  );
};
