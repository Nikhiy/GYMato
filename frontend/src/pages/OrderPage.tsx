...
const OrderPage = () => {
  ...
  return <div className="mx-auto max-w-3xl px-4 py-6 space-y-6">
    <h1 className="text-xl font-bold">Order #{order._id.slice(-6)}</h1>
    <div className="rounded-lg bg-blue-50 p-3 text-sm font-medium">Status:<span className="capitalize">{order.status}</span>
    </div>
    ...
    <div className="rounded-xl bg-white p-4 shadow-sm space-y-2">
        ...
        {
            (order.status==="rider_assigned" || order.status==="picked_up") && (riderLocation ? <UserOrderMap riderLocation={riderLocation} deliveryLocation={[order.deliveryAddress.latitude!,order.deliveryAddress.longitude]} /> : <p>Waiting for rider location...</p>
        )}
