...

    {
        order.paymentStatus==="paid" && actions.length>0 && (
            <div className="flex flex-wrap gap-2 pt-2">
                {actions.map((status)=>(
                    <button key={status} disabled={loading} onClick={()=>updateStatus(status)} className="rounded-lg bg-[#e23744] px-3 py-4 text-xs text-white hover:bg-[#32f3a0] disabled:opacity-50" >Mark as {status.replaceAll("_"," ")}</button>
                ))}
            </div>
        )
    }
